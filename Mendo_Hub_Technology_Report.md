...
| **Compliance & governance** | **Needs work** | The flags here are process items, not software defects: the AI services and an operational email account are on personal/unofficial arrangements rather than company ones, and the Bloomberg licence should be confirmed to cover how the data is being displayed and exported. None are difficult to fix; all should be closed as usage grows. Details in Section 6. |
...
**Morning Notes** deserves a note on how it works, since it's the most "AI" of the tools: each morning, an automated job collects the newsletters the team subscribes to (free market-commentary emails), sends the text to an AI service along with the team's watchlist of tickers so the summary highlights relevant names, and publishes a structured note — top themes plus watchlist mentions — to the site and to the team's inboxes.
...
4. **On schedule, cloud jobs collect newsletters and tweets** and send the text to AI services for summarization and theme-tagging, along with the team's watchlist of tickers so the output highlights relevant names.
...
|---|---|---|---|---|---|
| 1 | **AI services on personal/informal arrangements** | The AI coding assistant used to build and maintain the app runs on a personal subscription, and the AI services the app calls daily (news summarization, tweet analysis) are on standard self-serve terms | The pipelines routinely send the AI vendors the text of subscribed newsletters and public tweets, plus the team's **watchlist of tickers** (so summaries highlight relevant names). The content is not sensitive client data — it is free newsletters and public social posts — but the watchlist signals the team's areas of interest. Enterprise agreements add contractual commitments: the vendor won't train models on your data and won't retain it | **Medium** | Enterprise subscriptions for both the coding assistant and the AI services the app calls |
...
## The audit found one critical third-party vulnerability and a handful of robustness gaps; all were fixed, tested and deployed during the review itself
...
| 7 | No automated tests, no automated code-checking, no dependency monitoring | Nothing would catch a regression — or the next vulnerability like #1 | — | Added 33 tests, ESLint, continuous integration on every change, and Dependabot monitoring |

**The broader takeaway for the reader:** items 2–6 are quality hardening, but item 1 is the important story. It was not a flaw in anything the team built — it was a published vulnerability in a third-party component, the kind that affects thousands of companies at once and gets exploited within days of publication. The lasting fix is not the patch; it's the monitoring added in item 7, which turns "we find out when someone runs an audit" into "we find out automatically, with a fix proposed, usually the day the vulnerability is published."
