...
# ---------------------------------------------------------------------------
# Position list — sourced from the live /api/portfolio endpoint (port=1
# companies from the Equities Dashboard DB), with a static-file fallback.
# ---------------------------------------------------------------------------

def _get_positions() -> list[str]:
    # Primary: call the live site's portfolio API so the pipeline always uses
    # the same portfolio list that the dashboard displays.
    api_url = _env("PORTFOLIO_API_URL")
    if api_url:
        try:
            import urllib.request
            with urllib.request.urlopen(api_url.rstrip("/") + "/api/portfolio", timeout=10) as resp:
            ...
            print(f"WARN: could not fetch {api_url}/api/portfolio: {exc}", file=sys.stderr)
    # Fallback: read the committed portfolio.json (domain-hint file).
    try:
        with open(PORTFOLIO_JSON) as f:
            data = json.load(f)
        positions = data.get("positions", [])
        if positions:
            print("WARN: using static portfolio.json fallback", file=sys.stderr)
            return [
                f"{p['ticker']} ({p['name']})" if p.get("name") else p["ticker"]
                for p in positions
                if p.get("ticker")
